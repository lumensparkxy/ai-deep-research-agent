...
class TestResearchEngineErrorHandling:
    ...
    def test_research_engine_stage_failure_recovery(self, mock_settings):
        ...
        with patch('google.genai.Client') as mock_client_class:
            ...
            with patch.object(engine.validator, 'validate_query', return_value="valid query"), \
                 ...
                 patch.object(engine.session_manager, 'update_session_stage') as mock_update_stage, \
                 patch.object(engine.session_manager, 'update_session_conclusions') as mock_update_conclusions, \
                ...
                # The fallback result is appended to stages
                assert len(result["stages"]) > 0
                fallback_stage = result["stages"][0]
                assert "error" in fallback_stage
                assert "Stage 1 failed" in fallback_stage["error"]
    ...
    def test_research_engine_safety_settings_validation(self, mock_settings):
        ...
        with patch('google.genai.Client') as mock_client_class:
            ...
            engine._call_gemini_with_retry("test")
            
            # Verify generate_content was called with config containing safety settings
            mock_client.models.generate_content.assert_called_once()
            call_kwargs = mock_client.models.generate_content.call_args[1]
            assert "config" in call_kwargs
            assert call_kwargs["config"].safety_settings is not None
    ...
    def test_research_engine_memory_management(self, mock_settings):
        ...
        with patch('google.genai.Client') as mock_client_class:
            ...
            mock_client.models.generate_content.return_value = large_response
